...
export class AuthService {
  ...
  async register(registerDto: RegisterDto, response: Response) {
    const admin = await this.masterService.getAdminUser(registerDto?.emailId);

    if (admin) {
      return { message: ErrorMessage.ACCOUNT_EXISTS };
    }
    const createAdmin = await this.masterService.createAdminUser(registerDto);
  ...
  async verifyOtp(jwtDecoded: JwtResponseInterface, verifyDto: VerifyDto) {
    ...
    if (isValid) {
      ...
      await this.masterService.updateAdminUser(jwtDecoded?.id, {
  ...
  async sendOtp(userId: string, emailId: string) {
    ...
    await this.redisService.setCache(`otp:${userId}`, otp);
